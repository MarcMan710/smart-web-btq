                    ...
                    <tbody>
                        {Array.isArray(recordings) && recordings.map((recording) => (
                            <tr key={recording._id} className='odd:bg-nwhite2 even:bg-nwhite3'>
                                ...
                                <td className='px-4 py-2'>{new Date(recording.recordedAt).toLocaleString('id-ID', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' })}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
